"""Meeting workflow seeding and transition helpers."""
...
def _meeting_content_type():
    from apps.meetings.models import Meeting
    return ContentType.objects.get_for_model(Meeting)


@transaction.atomic
def ensure_meeting_workflow() -> None:
    """Create follow-up states/transitions if missing (safe to call repeatedly)."""
    ct = _meeting_content_type()
    state_map: dict[str, State] = {}

    for cfg in MEETING_STATES:
    ...
    for src_slug, dst_slug, label in MEETING_TRANSITIONS:
...
def assign_initial_state(meeting) -> None:
    ct = _meeting_content_type()
    if meeting.workflow_state_id:
        canonical = _canonical_state(ct, meeting.workflow_state)
        if canonical and canonical.id != meeting.workflow_state_id:
            meeting.workflow_state = canonical
            meeting.save(update_fields=["workflow_state"])
        return

    initial = State.objects.filter(content_type=ct, is_initial=True).order_by("order").first()
    if initial:
        meeting.workflow_state = initial
        meeting.save(update_fields=["workflow_state"])


def get_allowed_destination_slugs(meeting, user=None) -> list[str]:
    ensure_meeting_workflow()
    assign_initial_state(meeting)

    ct = _meeting_content_type()
    current = _canonical_state(ct, meeting.workflow_state)
    if not current:
        return []
...
@transaction.atomic
def proceed_meeting(meeting, user, destination_slug: str, comments: str = "") -> Proceeding | None:
    ensure_meeting_workflow()
    assign_initial_state(meeting)
    ...
    current = _canonical_state(ct, meeting.workflow_state)
    if current is None:
        raise WorkflowTransitionError("Meeting has no workflow state.")

    if current.content_type_id != ct.id:
        meeting.workflow_state = current
        meeting.save(update_fields=["workflow_state"])
    ...
    meeting.workflow_state = destination
    meeting.save(update_fields=["workflow_state"])
