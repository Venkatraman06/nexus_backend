...
class TokenView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.warning("Keycloak is unreachable (%s); falling back to local Django DB auth.", exc)
            ...
            payload = {
                "sub": emp.keycloak_id or str(emp.id),
            ...
            local_token = jwt.encode(payload, settings.SECRET_KEY, algorithm="HS256")
            token_data = {
        ...
        if emp:
            ...
            emp.last_login = django_timezone.now()
            emp.save(update_fields=["last_login"])

            # Resolve permissions
            permissions = []
            if not is_keycloak:
                group_name = emp.keycloak_group
                if group_name:
                    import json
                    import os
                    ...
                    if os.path.exists(role_path):
                        try:
                            with open(role_path, "r", encoding="utf-8") as rf:
                                ...
                                permissions = role_perms_map.get(group_name, [])
                        except Exception as e:
                            logger.error("Failed to load local role permissions: %s", e)
            else:
                from packages.keycloak.permissions import PermissionResolver
                user_id = emp.keycloak_id or str(emp.id)
                permissions = PermissionResolver().resolve_permissions(user_id)
